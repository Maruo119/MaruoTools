...
def save_results_in_background(data):
    """バックグラウンドで保存処理を行う関数"""
    try:
        # ファイルに保存（例: JSONとして保存）
        file_path = 'saved_results.json'
...
def save():
    try:
        ...
        # 必要なデータを確認
        if not all([keyword, repo_path, results]):
            return jsonify({'message': 'データが不足しています'}), 400
        ...
        # 保存処理を非同期で実行
        thread = threading.Thread(target=save_results_in_background, args=(results_data,))
        thread.start()

        return jsonify({'message': '保存処理を開始しました'}), 202
    except Exception as e:
...
@app.route('/')
def input_page():
    return render_template('input_v10.html')
...
def grep():
    ...
    return render_template('results_v10.html', results=results_data)
...
def file_content():
    ...
    return render_template("result_file_content_v10.html", file_path=file_path, content=content, keyword=keyword, repo_path=repo_path, file_type_include=file_type_include, file_type_exclude=file_type_exclude)
