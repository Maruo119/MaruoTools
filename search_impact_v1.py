...
def search_keyword_in_repository(repo_path, keyword, output_excel):
    ...
    # Add headers
    headers = ["File Path", "Line Number", "Line Content"]
    for col_num, header in enumerate(headers, start=1):
        col_letter = get_column_letter(col_num)
        sheet[f"{col_letter}1"] = header

    # Add data
    for row_num, (file_path, line_number, line_content) in enumerate(results, start=2):
        sheet[f"A{row_num}"] = file_path
        sheet[f"B{row_num}"] = line_number
        sheet[f"C{row_num}"] = line_content
...
if __name__ == "__main__":
    ...
    output_excel_path = "/Users/yuta/backlog_python/search_impact/output.xlsx".strip()
